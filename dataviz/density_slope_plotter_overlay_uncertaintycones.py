        ...
        for moisture_level in self.group_slopes:
            for compaction_level in self.group_slopes[moisture_level]:
                for group in self.group_slopes[moisture_level][compaction_level]:
                    ...
                    for slope in slopes:
                        # Percentage uncertainty (example: ±10%)
                        pct = 0.10
                        lower = slope * (1 - pct)
                        upper = slope * (1 + pct)
...
# ---------------------------------------------------------
        ...
        for moisture, data in moisture_groups.items():
            ...
            predicted = a*densities**2 + b*densities
            ...
            k=2
            band = k * std_residuals

            x_line = np.linspace(0, max(densities), 200)
            y_line = a * x_line**2 + b * x_line
            y_line_upper = y_line + band
            y_line_lower = y_line - band
            ...
            plt.plot(x_line, y_line_upper, color=color, linewidth=2, linestyle='--')
            plt.plot(x_line, y_line_lower, color=color, linewidth=2, linestyle='--')
